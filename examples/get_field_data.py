...
def get_field1D(path,component,n_prc,step):
    """Gather the output data of 1D simulations and return them as a numpy array.
    ...
    for prc in range(n_prc):
        tmp = np.array(pd.read_csv(path+f"/{step}_{prc}.csv",header=None)[component])
        temp.append(tmp)
...
### Wrapper to combine multiple steps
def combine_steps(function,*args,steps):
    """Wrapper to combine the field values of multiple output steps into one array.
    
    The extracted field values using a get_field function are concatenated to one array.
    
    Parameters
    -----------
    function : function
        get_field function used to extract a field component from the output csv file.
    *args : ...
