...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s1f(sunrealtype const *udata, const int dPD) {
  return -1.0 / 1.0 * udata[-1 * dPD] + udata[0];
}
// Upwind (backward) differentiating
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s1b(sunrealtype const *udata, const int dPD) {
  return -1.0 / 1.0 * udata[0] + udata[1 * dPD];
}

#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s2f(sunrealtype const *udata, const int dPD) {
  return 1.0 / 2.0 * udata[-2 * dPD] - 2.0 / 1.0 * udata[-1 * dPD] +
         3.0 / 2.0 * udata[0];
}
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s2c(sunrealtype const *udata, const int dPD) {
  return -1.0 / 2.0 * udata[-1 * dPD] + 0 + 1.0 / 2.0 * udata[1 * dPD];
}
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s2b(sunrealtype const *udata, const int dPD) {
  return -3.0 / 2.0 * udata[0] + 2.0 / 1.0 * udata[1 * dPD] -
         1.0 / 2.0 * udata[2 * dPD];
}
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s3f(sunrealtype const *udata, const int dPD) {
  return 1.0 / 6.0 * udata[-2 * dPD] - 1.0 / 1.0 * udata[-1 * dPD] +
         1.0 / 2.0 * udata[0] + 1.0 / 3.0 * udata[1 * dPD];
}
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s3b(sunrealtype const *udata, const int dPD) {
  return -1.0 / 3.0 * udata[-1 * dPD] - 1.0 / 2.0 * udata[0] + udata[1 * dPD] -
         1.0 / 6.0 * udata[2 * dPD];
}
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s4f(sunrealtype const *udata, const int dPD) {
  return -1.0 / 12.0 * udata[-3 * dPD] + 1.0 / 2.0 * udata[-2 * dPD] -
         3.0 / 2.0 * udata[-1 * dPD] + 5.0 / 6.0 * udata[0] +
         1.0 / 4.0 * udata[1 * dPD];
}
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s4c(sunrealtype const *udata, const int dPD) {
  return 1.0 / 12.0 * udata[-2 * dPD] - 2.0 / 3.0 * udata[-1 * dPD] + 0 +
         2.0 / 3.0 * udata[1 * dPD] - 1.0 / 12.0 * udata[2 * dPD];
}
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s4b(sunrealtype const *udata, const int dPD) {
  return -1.0 / 4.0 * udata[-1 * dPD] - 5.0 / 6.0 * udata[0] +
         3.0 / 2.0 * udata[1 * dPD] - 1.0 / 2.0 * udata[2 * dPD] +
         1.0 / 12.0 * udata[3 * dPD];
}
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s5f(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s5b(sunrealtype const *udata, const int dPD) {
  return 1.0 / 20.0 * udata[-2 * dPD] - 1.0 / 2.0 * udata[-1 * dPD] -
         1.0 / 3.0 * udata[0] + udata[1 * dPD] - 1.0 / 4.0 * udata[2 * dPD] +
         1.0 / 30.0 * udata[3 * dPD];
}
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s6f(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s6c(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s6b(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s7f(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s7b(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s8f(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s8c(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s8b(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s9f(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s9b(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s10f(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s10c(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s10b(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s11f(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s11b(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s12f(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s12c(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s12b(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s13f(sunrealtype const *udata, const int dPD) {
...
#pragma omp declare simd uniform(dPD) notinbranch
inline sunrealtype s13b(sunrealtype const *udata, const int dPD) {
...
inline sunrealtype s1f(sunrealtype const *udata) { return s1f(udata, 6); }
inline sunrealtype s1b(sunrealtype const *udata) { return s1b(udata, 6); }
inline sunrealtype s2f(sunrealtype const *udata) { return s2f(udata, 6); }
inline sunrealtype s2c(sunrealtype const *udata) { return s2c(udata, 6); }
inline sunrealtype s2b(sunrealtype const *udata) { return s2b(udata, 6); }
inline sunrealtype s3f(sunrealtype const *udata) { return s3f(udata, 6); }
inline sunrealtype s3b(sunrealtype const *udata) { return s3b(udata, 6); }
inline sunrealtype s4f(sunrealtype const *udata) { return s4f(udata, 6); }
inline sunrealtype s4c(sunrealtype const *udata) { return s4c(udata, 6); }
inline sunrealtype s4b(sunrealtype const *udata) { return s4b(udata, 6); }
inline sunrealtype s5f(sunrealtype const *udata) { return s5f(udata, 6); }
inline sunrealtype s5b(sunrealtype const *udata) { return s5b(udata, 6); }
inline sunrealtype s6f(sunrealtype const *udata) { return s6f(udata, 6); }
inline sunrealtype s6c(sunrealtype const *udata) { return s6c(udata, 6); }
inline sunrealtype s6b(sunrealtype const *udata) { return s6b(udata, 6); }
inline sunrealtype s7f(sunrealtype const *udata) { return s7f(udata, 6); }
inline sunrealtype s7b(sunrealtype const *udata) { return s7b(udata, 6); }
inline sunrealtype s8f(sunrealtype const *udata) { return s8f(udata, 6); }
inline sunrealtype s8c(sunrealtype const *udata) { return s8c(udata, 6); }
inline sunrealtype s8b(sunrealtype const *udata) { return s8b(udata, 6); }
inline sunrealtype s9f(sunrealtype const *udata) { return s9f(udata, 6); }
inline sunrealtype s9b(sunrealtype const *udata) { return s9b(udata, 6); }
inline sunrealtype s10f(sunrealtype const *udata)
{ return s10f(udata, 6); }
inline sunrealtype s10c(sunrealtype const *udata)
{ return s10c(udata, 6); }
inline sunrealtype s10b(sunrealtype const *udata)
{ return s10b(udata, 6); }
inline sunrealtype s11f(sunrealtype const *udata)
{ return s11f(udata, 6); }
inline sunrealtype s11b(sunrealtype const *udata)
{ return s11b(udata, 6); }
inline sunrealtype s12f(sunrealtype const *udata)
{ return s12f(udata, 6); }
inline sunrealtype s12c(sunrealtype const *udata)
{ return s12c(udata, 6); }
inline sunrealtype s12b(sunrealtype const *udata)
{ return s12b(udata, 6); }
inline sunrealtype s13f(sunrealtype const *udata)
{ return s13f(udata, 6); }
inline sunrealtype s13b(sunrealtype const *udata)
{ return s13b(udata, 6); }
